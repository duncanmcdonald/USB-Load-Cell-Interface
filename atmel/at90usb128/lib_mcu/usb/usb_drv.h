...
//!
//! @brief Atmel file. USB low level driver definitions.
//!
//! @version 1.34 at90usb128-demo-template-1_0_5 $Id: usb_drv.h,v 1.34 2006/07/27 07:03:41 rletendu Exp $
//!
//! @todo
//! @bug
...
//! Part of Setup transaction. Performs the handshake stage ACK
...
//! sends IN on control endpoint; Interrupt set 
//! when SIE is ready to accept data. 
//! Cleared by sw to handshake the interrupt.
#define Usb_send_control_in()                     (UEINTX &= ~(1<<TXINI)) // TXINI = 0; resets TXINI
//! frees OUT bank
...
//! @brief enables host device connection interrupt
#define Host_enable_device_connection_interrupt()        (UHIEN |= (1<<DCONNE))
//! @brief disables USB device connection interrupt
...
//! @brief tests if a USB device has been detected
#define Is_device_connection()                 (UHINT & (1<<DCONNI))
//! @brief acks device connection
#define Host_ack_device_connection()           (UHINT = ~(1<<DCONNI))

//! @brief enables host device disconnection interrupt
